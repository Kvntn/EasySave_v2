            ...
            switch (operationType)
            {
                ...
                case 2:
                    if (jsonLength > 0)
                    {
                        ...
                        Console.WriteLine("* Execute certain backup from backups list. \n");
        ...
        public static void ExistingBackup(int BackupTypeInt)    // used to launch the Backups
        {
            ...
            switch (BackupTypeInt)
            {
                ...
                incorrectTaskname:
                    ...
                    while (pursue == true)
                    {
                        Console.WriteLine("\nCannot execute backup while having a background process!");
                        Thread.Sleep(3000);
                ...
                case 2:
                    ...
                    foreach (var Backup in BackupsList)
                    {
                        ...
                        Process[] checkProcess2 = Process.GetProcessesByName("Calculator");   // check process calculator

                        if (checkProcess2.Length > 0)
                        {
                            pursue = true;
                        }

                        while (pursue == true)      // check if background process still in use
                        {
                            Console.WriteLine("Cannot execute backup while having a background process!");
                            Thread.Sleep(3000);
                            Process[] recheck = Process.GetProcessesByName("Calculator");   // recheck process
                            if (recheck.Length == 0)
                            {
                                pursue = false;
                            }
                        ...
                        int totalPerBackup = source.GetFiles("*", SearchOption.AllDirectories).Length;
                        SeqtotalFiles += totalPerBackup;
